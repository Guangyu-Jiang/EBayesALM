#' select grid points
#' Input: observations X R^{n*d} (n observations in each row)
...
#' Output: grid points U R^{m*d} (m observations in each row)
#' @export
select_grid <- function(X, grid_option, m){
  ...
  if (grid_option == 3){
    ...
    my <- round(sqrt(m))
    mx <- round(sqrt(m))
    xgrid <- seq(from = xmin, to = xmax, length.out = mx)
    ygrid = seq(from = ymin, to = ymax, length.out = my)
